...
const signupSchema = z.object({
  ...
  password: z.string()
    ...
    .regex(/(?:(?=.*\d)|(?=.*\W+))(?=.*[A-Z])(?=.*[a-z])/, 'Password must contain uppercase, lowercase, and either a number or special character'),
...
const SignupModal = ({ isOpen, onClose, onSwitchToLogin }: SignupModalProps) => {
  ...
  const passwordRequirements = [
    { label: 'At least 8 characters', met: password?.length >= 8 && password?.length <= 20 },
    { label: 'One uppercase letter', met: /[A-Z]/.test(password || '') },
    { label: 'One lowercase letter', met: /[a-z]/.test(password || '') },
    { label: 'One number or special character', met: /\d/.test(password || '') || /\W/.test(password || '') },
  ];
